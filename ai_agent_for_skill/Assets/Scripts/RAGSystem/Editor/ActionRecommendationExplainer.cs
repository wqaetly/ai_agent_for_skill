...
namespace SkillSystem.RAG
{
    ...
    public class ActionRecommendationExplainer
    {
        ...
        // 相似度阈值配置
        private const float HIGH_SIMILARITY_THRESHOLD = 0.7f;
            ...
            List<string> existingActions = null)
        {
            ...
            // 添加参考技能
            GenerateReferenceSkills(recommendation);
        }
        ...
        private void GenerateReasons(EnhancedActionRecommendation recommendation, string context)
        {
            ...
            {
                recommendation.reasons.Add($"低语义相似度（{recommendation.semantic_similarity:P0}），可能不完全匹配查询意图");
            }
            ...
            if (semanticInfo != null)
            {
                ...
                // 理由5：协同效果
                if (semanticInfo.dependency?.synergies != null && semanticInfo.dependency.synergies.Count > 0)
                {
            ...
            {
                recommendation.reasons.Add("基于语义检索推荐");
            }
        }
        ...
        private void GenerateWarnings(EnhancedActionRecommendation recommendation, List<string> existingActions)
        {
            ...
            // 警告1：验证问题
            if (recommendation.validation_issues != null && recommendation.validation_issues.Count > 0)
            {
            ...
            // 警告2：互斥关系
            if (existingActions != null && existingActions.Count > 0 && semanticInfo != null)
            {
            ...
            // 警告3：缺少前置
            if (semanticInfo?.dependency?.prerequisites != null && semanticInfo.dependency.prerequisites.Count > 0)
            {
            ...
            // 警告4：低相似度
            if (recommendation.semantic_similarity < MEDIUM_SIMILARITY_THRESHOLD)
            {
                recommendation.warnings.Add("⚠ 语义相似度较低，请确认是否符合需求");
            }
        }
        ...
        private void GenerateSuggestions(EnhancedActionRecommendation recommendation, List<string> existingActions)
        {
            ...
            // 建议4：参数配置提示
            AddParameterSuggestions(recommendation);
        }
        ...
        private void AddParameterSuggestions(EnhancedActionRecommendation recommendation)
        {
            ...
            {
                case "DamageAction":
                    recommendation.suggestions.Add("💡 注意配置：伤害类型、伤害值、目标筛选");
                    break;
                case "MovementAction":
                    recommendation.suggestions.Add("💡 注意配置：移动类型、移动速度、目标位置");
                    break;
                case "ShieldAction":
                    recommendation.suggestions.Add("💡 注意配置：护盾类型、护盾值、持续时间");
                    break;
                case "HealAction":
                    recommendation.suggestions.Add("💡 注意配置：治疗值、目标筛选、是否瞬时生效");
                    break;
                case "ControlAction":
                    recommendation.suggestions.Add("💡 注意配置：控制类型、持续时间、是否可被打断");
                    break;
                case "BuffAction":
        ...
        {
            // 这里可以从技能数据库中查询使用了该Action的技能作为参考
            // 当前先添加一些通用的参考说明
            var semanticInfo = registry.GetSemanticInfo(recommendation.action_type);
            ...
            {
                // 基于场景推荐参考技能
                foreach (var scenario in semanticInfo.purpose.scenarios.Take(2))
                {
                    recommendation.reference_skills.Add($"参考{scenario}的实现方式");
                }
            }
        }
        ...
        private string GetCategoryUsage(string category)
        {
            var usages = new Dictionary<string, string>
            {
                { "Damage", "用于造成伤害的技能效果" },
                { "Heal", "用于恢复生命值的技能效果" },
                { "Shield", "用于提供护盾保护的技能效果" },
                { "Defense", "用于防御和保护的技能效果" },
                { "Movement", "用于改变角色位置的技能效果" },
                { "Control", "用于控制目标行为的技能效果" },
                { "Buff", "用于提升属性的技能效果" },
                { "Debuff", "用于降低属性的技能效果" },
                { "Summon", "用于召唤单位的技能效果" },
                { "Visual", "用于视觉表现的技能效果" },
        ...
        public string GenerateSummaryText(EnhancedActionRecommendation recommendation)
        {
            ...
            {
                parts.Add($"警告：{recommendation.warnings.Count}项");
            }
