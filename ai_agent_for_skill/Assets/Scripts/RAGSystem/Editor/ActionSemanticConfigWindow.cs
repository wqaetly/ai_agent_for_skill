...
namespace SkillSystem.RAG
{
    ...
    public class ActionSemanticConfigWindow : EditorWindow
    {
        ...
        private void DrawConfigInfo()
        {
            ...
            EditorGUILayout.LabelField($"已启用规�? {rules.Count}");
            EditorGUILayout.LabelField($"配置文件路径: {registry.GetConfigPath()}");
            EditorGUILayout.LabelField($"最后加载时�? {registry.GetLastLoadTime():yyyy-MM-dd HH:mm:ss}");
        ...
        private void DrawConfigManagement()
        {
            ...
            if (GUILayout.Button("重新加载配置", GUILayout.Height(30)))
            {
                ...
                {
                    ShowNotification(new GUIContent("配置已重新加�?));
                }
                else
                {
            ...
            if (GUILayout.Button("保存配置", GUILayout.Height(30)))
            {
                ...
                {
                    ShowNotification(new GUIContent("配置已保�?));
                }
                else
                {
            ...
            if (GUILayout.Button("在编辑器中打开", GUILayout.Height(30)))
            {
                ...
                {
                    EditorUtility.DisplayDialog("提示", "配置文件不存在，将自动创�?, "确定");
        ...
        {
            EditorGUILayout.BeginVertical("box");
            EditorGUILayout.LabelField("系统健康检�?, EditorStyles.boldLabel);
            EditorGUILayout.Space(5);

            if (GUILayout.Button("执行健康检�?, GUILayout.Height(30)))
            {
                string message;
                ...
                if (isHealthy)
                {
                    EditorUtility.DisplayDialog("健康检�?, message, "确定");
                }
                else
                {
                    EditorUtility.DisplayDialog("健康检查失�?, message, "确定");
                }
            }
        ...
        private void DrawTestTools()
        {
            ...
            EditorGUILayout.LabelField("测试上下�?");
        ...
        private void TestScoringSystem()
        {
            ...
            testResult = "=== 评分系统测试 ===\n";
            testResult += $"测试上下�? {testContext}\n\n";

            foreach (var rec in enhanced)
            {
                testResult += $"--- {rec.action_type} ---\n";
                testResult += $"语义相似�? {rec.semantic_similarity:P0}\n";
                testResult += $"业务得分: {rec.business_score:F2}\n";
                testResult += $"最终得�? {rec.final_score:P0}\n";
                testResult += $"验证状�? {(rec.is_valid ? "通过" : "失败")}\n";
