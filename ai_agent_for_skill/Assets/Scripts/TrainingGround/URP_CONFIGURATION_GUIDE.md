...
Renderer Features:
  - 添加以下Renderer Features（如需要）:
    ...
    * Screen Space Reflections (SSR) - 可选
```
...
1. 确认URP包已正确安装
2. 检查Shader名称拼写是否正确
3. 重新导入URP包: `Window → Package Manager → URP → Reimport`

### 5.2 Post-Processing不生效
...
- [ ] DamageNumberPool有有效的预制体
- [ ] Canvas的World Space相机已设置
- [ ] TextMeshPro已正确导入
...
### 基础设置
- [ ] URP包已安装（版本17.1.0）
- [ ] Cinemachine包已安装（版本3.0.1）
- [ ] 项目Graphics设置指向URP Asset

### 场景组件
- [ ] 主相机存在并配置正确
- [ ] CinemachineBrain已添加到主相机
- [ ] TrainingGroundCameraController已添加到场景
- [ ] Post-Processing Volume已创建

### 训练场组件
- [ ] TrainingGroundManager已添加到场景
- [ ] DamageNumberPool已创建
- [ ] MaterialLibrary自动初始化
- [ ] PostProcessingManager已添加
