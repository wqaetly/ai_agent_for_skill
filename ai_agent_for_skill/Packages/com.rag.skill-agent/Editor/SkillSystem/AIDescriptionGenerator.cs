...
namespace RAG
{
    ...
    public class AIDescriptionGenerator
    {
        ...
        public string GetActionSourceCode(string typeName)
        {
            try
            {
                // 使用配置驱动的方式查找类型，避免直接依赖 ISkillAction
                Type actionType = null;
                var baseActionType = config.skillSystemConfig.GetBaseActionType();

                if (baseActionType != null)
                {
                    // 从基类所在程序集中查找
                    actionType = Assembly.GetAssembly(baseActionType)?
                        .GetTypes()
                ...
                if (actionType == null)
                {
                    foreach (var assembly in AppDomain.CurrentDomain.GetAssemblies())
                    {
                        try
                        {
                            actionType = assembly.GetTypes().FirstOrDefault(t => t.Name == typeName);
