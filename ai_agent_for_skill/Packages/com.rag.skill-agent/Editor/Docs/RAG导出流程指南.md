...
在仓库根目录运行 `launch.bat`，选菜单 `[1] Full System` 会同时拉起：

- **Langflow** （fork: wqaetly/langflow @ dev）在端口 **7860**，提供可视化工作流编排与调试 Playground
- **OpenAI 兼容适配层**（`skill_agent/openai_compat`）在端口 **2024**，提供：
  - `GET /v1/models` 返回 Langflow flow 列表
  - `POST /v1/chat/completions` 流式调用对应 flow
- **Lobe Chat** 在端口 **3210**，作为面向策划的聊天前端
...
导出新数据后，需要重建索引：
- **自动**：开启“导出后自动通知重建索引”
- **手动**：在 `skill_agent/` 下运行 `python rebuild_index.py`
---
