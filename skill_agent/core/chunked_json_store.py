"""
ChunkedJsonStore - 大JSON文件流式加载�?支持按路径加载指定片段，限制内存占用
"""
...
class ChunkedJsonStore:
    """流式JSON存储，支持按路径访问大文件片�?""

    def __init__(self, max_chunk_size_mb: float = 10.0):
        """
        Args:
            max_chunk_size_mb: 单次加载的最大内存限�?MB)
        """
    ...
    def load_by_path(
        ...
        Args:
            file_path: JSON文件路径
            json_path: JSONPath表达式，�?"tracks.$rcontent[2].actions.$rcontent[0]"
            include_context: 是否包含父级上下文信�?
        Returns:
            提取的JSON片段，包含元数据
        ...
        # 按路径提取数�?        data = self._extract_by_path_parts_dict(full_data, path_parts)
    ...
    def get_chunk_summary(
        ...
        if isinstance(data, dict):
            if "$type" in data:
                # Odin序列化对�?                return self._summarize_odin_object(data, chunk.get("context"))
            else:
                return self._summarize_dict(data)
        elif isinstance(data, list):
            return f"数组，包含{len(data)}个元�?
        else:
            return str(data)
    ...
    def _extract_by_path_parts_dict(
        ...
        for key, index in path_parts:
            ...
            # 访问�?            if isinstance(current, dict) and key in current:
                current = current[key]
            else:
                return None
    ...
    def _find_line_number_by_path(
        ...
        """
        查找路径对应的行号范�?
        这需要记录JSON解析器的当前位置，比较复�?        这里使用简化方案：通过文本搜索估算
        """
        file_obj.seek(0)
        ...
        # 将目标数据转为紧凑JSON字符�?        target_json = json.dumps(target_data, ensure_ascii=False, separators=(',', ':'))

        # 在文件中搜索（简化版，实际需要更精确的方法）
        # 这里只是估算，真实实现需要使用JSON解析器的位置信息
        start_pos = content.find(target_json[:50])  # 搜索�?0字符
        if start_pos == -1:
            return None
    ...
    def _extract_context(
        ...
        for i, (key, index) in enumerate(path_parts):
            if key == "tracks":
                # 找到tracks后面的索�?                if i + 1 < len(path_parts) and path_parts[i+1][1] is not None:
    ...
    def _summarize_dict(self, data: Dict[str, Any]) -> str:
        ...
        if len(data) > 5:
            return f"包含{len(data)}个字�? {', '.join(keys)}..."
        else:
