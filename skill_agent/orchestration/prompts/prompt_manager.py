"""
Prompt 管理�?加载和管理所�?Prompt 模板，支持变量替�?"""
...
class PromptManager:
    ...
    def __init__(self, prompts_file: str = None):
        ...
        初始�?Prompt 管理�?
        Args:
            prompts_file: prompts.yaml 文件路径，默认使用当前目录下�?        """
        ...
        if not self.prompts_file.exists():
            raise FileNotFoundError(f"Prompts 文件不存�? {self.prompts_file}")
    ...
    def get_prompt(self, prompt_key: str, **kwargs) -> ChatPromptTemplate:
        ...
        Args:
            prompt_key: Prompt 键名（如 'skill_generation'�?            **kwargs: 变量值（用于预填充）

        Returns:
            ChatPromptTemplate 实例
        """
        if prompt_key not in self.prompts:
            raise KeyError(f"Prompt '{prompt_key}' 不存�?)
...
# 全局 Prompt 管理器实�?_global_prompt_manager = None
