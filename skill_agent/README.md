...
skill_agent（Skill Retrieval-Augmented Generation）是一个基�?*检索增强生成（RAG�?*技术的智能技能辅助系统。它通过向量化技能数据，提供语义搜索和智能推荐，帮助开发者快速找到相似技能、获取参数建议，大幅提升技能设计效率�?

### 🎯 核心价�?

- **🔍 语义搜索**：用自然语言描述查找相似技能，无需记忆技能ID
- **💡 智能推荐**：根据上下文自动推荐Action类型和参数配�?
- **📚 知识沉淀**：将所有技能经验转化为可搜索的知识�?
- **�?实时同步**：技能文件修改后自动更新索引，始终保持最�?

### 🌟 适用场景

- **大规模技能库管理**：技能数量超�?00个，难以手动查找
- **团队协作开�?*：快速了解他人设计的技能，保持风格统一
- **参数调优**：参考相似技能的参数配置，加速平衡性调�?
- **学习与培�?*：新成员快速学习现有技能设计模�?
...
- **智能检查器**：Action Inspector中自动显示参数建�?
- **一键应�?*：直接将推荐参数填充到Action
- **无缝集成**：与现有技能编辑器完美配合
...
2. 模型文件获取方式�?
   - �?HuggingFace 下载：https://huggingface.co/Qwen/Qwen3-Embedding-0.6B
   - 使用 `git lfs` 克隆�?
     ```bash
     git lfs install
...
**注意**�?
- 需�?transformers>=4.51.0 才能支持Qwen3模型
- 需�?sentence-transformers>=3.0.0
- Qwen3支持100+语言，嵌入维�?024，性能优于前代模型
...
embedding:
  ...
  device: "cpu"  # �?"cuda" 如有GPU

# 技能索引配�?
skill_indexer:
  skills_directory: "../../ai_agent_for_skill/Assets/Skills"  # 修改为你的技能路�?
```
...
1. Unity菜单 �?**技能系�?/ RAG功能 / 启用RAG功能**
2. 菜单 �?**Edit / Preferences / 技能系�?/ RAG设置**
3. 点击**测试连接**
...
1. 打开RAG查询窗口（技能系�?�?RAG查询窗口�?
2. 输入�?带冲锋效果的伤害技�?
3. 点击搜索

**结果**�?
```
Riven Broken Wings (相似�? 87%)
- 文件: RivenBrokenWings.json
- 6轨道, 15动作, 150�?

Tryndamere Spinning Slash (相似�? 76%)
...
| 层级 | 技�?| 说明 |
|------|------|------|
| **Unity客户�?* | C# / UnityWebRequest | 编辑器集成和HTTP通信 |
| **Python服务** | FastAPI / Uvicorn | RESTful API服务�?|
| **嵌入模型** | Qwen3-Embedding-0.6B | 通义千问嵌入模型�?024维向量，支持100+语言�?|
| **向量数据�?* | ChromaDB | 本地持久化向量存�?|
| **文件监听** | Watchdog | 自动检测文件变�?|

### 核心模块

#### Python后端

- **embeddings.py**：文本向量化，缓存管�?
- **vector_store.py**：ChromaDB封装，向量CRUD
- **skill_indexer.py**：技能解析，索引构建
- **rag_engine.py**：RAG核心逻辑，检索排�?
...
�?  └── RAGEditorIntegration.cs  # 编辑器集�?
�?
├── Data/                        # 数据存储
�?  ├── vector_db/               # ChromaDB数据�?
�?  ├── embeddings_cache/        # 嵌入模型缓存
�?  ├── skill_index.json         # 技能索引缓�?
�?  └── rag_server.log           # 服务器日�?
�?
├── Docs/                        # 完整文档
�?  ├── UserGuide.md             # 用户指南
...
| 文档 | 说明 |
|------|------|
| **[用户指南](Docs/UserGuide.md)** | 详细使用教程，从入门到精�?|
| **[API文档](Docs/API.md)** | RESTful API接口说明 |
| **[Action参考](Docs/ActionReference.md)** | 21种Action类型的参数详�?|
| **[技能模式](Docs/SkillPatterns.md)** | 20种常见技能设计模�?|
...
# 技能索引配�?
skill_indexer:
  skills_directory: "../../ai_agent_for_skill/Assets/Skills"  # 修改此路�?
  watch_enabled: true  # 启用文件监听

# RAG配置
rag:
  top_k: 5  # 默认返回结果�?
  similarity_threshold: 0.5  # 相似度阈�?
...
**A**: 请检查：
1. 模型文件是否完整下载�?`Data/models/Qwen3-Embedding-0.6B/` 目录
2. 检�?config.yaml 中的 model_name 路径是否正确
3. 确保所有必需的模型文件都存在（config.json, model.safetensors 等）

### Q: 遇到 KeyError: 'qwen3' 错误�?

**A**: 需要升�?transformers 版本�?
```bash
...
- **Sentence-Transformers**: 优秀的嵌入模型框�?
- **ChromaDB**: 易用的向量数据库
