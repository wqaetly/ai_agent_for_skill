...
skill_agent是一个基�?*检索增强生成（RAG�?*技术的智能技能辅助系统，专为Unity技能编辑器设计。它能够�?

- 🔍 **语义搜索**：用自然语言描述查找相似技�?
- 💡 **智能推荐**：根据上下文推荐Action类型和参�?
- 📚 **知识�?*：索引所有技能文件，建立可搜索的知识�?
- �?**实时更新**：技能文件修改后自动更新索引
...
- **向量�?*: 将技能描述转换为数学向量
- **语义搜索**: 基于含义而非关键词的搜索
- **嵌入模型**: 使用多语言BERT模型支持中英�?
- **向量数据�?*: ChromaDB本地存储向量数据
...
```yaml
skill_indexer:
  skills_directory: "../../ai_agent_for_skill/Assets/Skills"  # 修改为你的技能目�?
```
...
1. 打开Unity项目
2. 菜单 �?**技能系�?/ RAG功能 / 启用RAG功能**
3. 菜单 �?**Edit / Preferences / 技能系�?/ RAG设置**
4. 点击**测试连接**确认连接成功
...
- "眩晕并减速敌�? �?ControlAction + BuffAction
...
**自动启用**（如果在Preferences中启�?自动显示参数建议"�?

在技能编辑器中选中任何Action时，会自动显�?🤖 AI参数建议"面板�?

#### 功能说明

1. **参数示例**: 显示相似技能中的实际参数配�?
2. **来源技�?*: 参数来自哪个技�?
3. **一键应�?*: 点击"应用"按钮直接填充参数
4. **批量应用**: 应用整套参数配置
...
1. 添加DamageAction到轨�?
2. 选中该Action
3. 查看AI建议的baseDamage、damageType等参�?
4. 点击"应用"按钮快速配�?

**示例2: 学习参数组合**

- 查看其他技能如何配置MovementAction
- 了解常用的movementSpeed值范�?
- 学习参数之间的配合关�?
...
**Edit / Preferences / 技能系�?/ RAG设置**

- **启用RAG功能**: 总开�?
- **自动显示参数建议**: 是否在ActionInspector中显示AI建议
- **服务器地址**: RAG服务器IP（默�?27.0.0.1�?
- **服务器端�?*: RAG服务器端口（默认8765�?
    ...
    (success, response, error) => {
        if (success) {
            foreach (var result in response.results) {
                Debug.Log($"找到: {result.skill_name} (相似�? {result.similarity})");
            }
        }
    }
));
