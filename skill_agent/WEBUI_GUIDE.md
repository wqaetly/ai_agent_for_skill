...
skill_agent WebUI 是一个基�?LangGraph �?agent-chat-ui 的可视化技能分析系统，让策划人员可以通过网页界面进行技能的分析、开发和修复工作�?
...
3. **环境变量**
   - `DEEPSEEK_API_KEY`: DeepSeek API 密钥（用�?LLM 调用�?
...
#### LangGraph 服务�?(langgraph_server.py)
...
详细 API 文档请访�? http://localhost:2024/docs

## 🔧 故障排除

### 问题 1: LangGraph 服务器启动失�?

**症状**: `ModuleNotFoundError` 或依赖缺�?
...
**解决方案**:
1. 确认 LangGraph 服务器正在运�? http://localhost:2024/health
2. 检�?`.env` 文件中的 `NEXT_PUBLIC_API_URL` 配置
3. 检查防火墙设置
...
- **LangGraph 服务器日�?*: `skill_agent/langgraph_server.log`
- **WebUI 日志**: 在启�?WebUI 的终端中查看
...
1. �?`skill_agent/orchestration/graphs/` 中创建新的图
2. �?`langgraph_server.py` 中注册新的助�?
3. 更新 `/assistants` 端点
